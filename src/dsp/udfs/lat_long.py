...
def get_lat_long(postcode, point_in_time=None):

    provider = ONSProvider()

    rec = provider.get(postcode)
    if not rec:
        return None

    eastings = rec.get_eastings_at(point_in_time)
    if not eastings:
        return None
    northings = rec.get_northings_at(point_in_time)
    if not northings:
        return None
    ...
    lat, long = OSGB36toWGS84(eastings, northings)
