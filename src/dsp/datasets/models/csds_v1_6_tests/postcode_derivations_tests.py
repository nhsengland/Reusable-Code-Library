...
from dsp.datasets.models import csds_v1_6
from dsp.datasets.models.csds_v1_6_tests.csds_v1_6_helper_tests import patient, mpi
...
def test_valid_postcode_flag(patient_post_code, expected_valid_postcode_flag):
    ...
    new_patient = csds_v1_6.MPI(record)  # creates a table
...
@pytest.mark.parametrize("rep_period_start_date, gp_practice_code, expected_org_id", [
    ...
    (date(2020, 1, 1), 'A81017', '00K'),
    (date(2020, 1, 1), 'D82001', '06V'),
    (date(2020, 1, 1), 'G82092', '99J'),
    (date(2020, 1, 1), 'bad practice code', None),
    (date(2020, 1, 1), None, None),
    (date(2023, 3, 31), 'A81017', '16C'),
    (date(2023, 4, 1), 'A81017', None),
])
def test_org_id_of_ccg_from_gp_practice(rep_period_start_date, gp_practice_code, expected_org_id):
    record = patient()
    record['Header']['RP_StartDate'] = rep_period_start_date
    record['GPPracticeRegistrations'][0]['OrgID_GP'] = gp_practice_code
    new_patient = csds_v1_6.MPI(record)  # creates a table
