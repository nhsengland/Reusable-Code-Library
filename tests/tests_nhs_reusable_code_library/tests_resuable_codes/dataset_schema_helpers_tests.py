...
from dsp.shared.common import concurrent_tasks
...
def test_get_cols(spark: SparkSession):
    ...
    table = "CYP001"
    spark.sql(
        f"""CREATE TABLE IF NOT EXISTS {temp_db}.{table} (
...
def test_create_view(spark: SparkSession):
    try:
        ...
        expected_view_statements = [
            f'CREATE VIEW `{target_db}`.`{table1}`(EFFECTIVE_FROM, EFFECTIVE_TO, Person_ID_Mother) \
                AS SELECT EFFECTIVE_FROM, EFFECTIVE_TO, Person_ID_Mother FROM {legacy_schema_name}.{table1} ',
            f'CREATE VIEW `{target_db}`.`{table2}`(LPI, Person_ID_Mother, name) \
                AS SELECT LPI, Person_ID_Mother, name FROM {current_schema_name}.{table2}   \
                    UNION ALL   SELECT LPI, Person_ID_Mother, name FROM {legacy_schema_name}.{table2} ',
            f'CREATE VIEW `{target_db}`.`{table3}`(LPI, SRV, name) \
                AS SELECT LPI, SRV, name FROM {current_schema_name}.{table3} '
        ]
        assert views == expected_view_statements, "Invalid Assertion - diff at index 0"
    except AssertionError as error:
        print(error)


@pytest.mark.parametrize("deid_type", ['pseudo', 'sensitive'])
def test_create_pseudo_sensitive_view(spark: SparkSession, deid_type: str):
    pseudo_db_test = "pseudo_db_test"
    pseudo = "pseudo"
    spark.sql(f"CREATE DATABASE IF NOT EXISTS {pseudo_db_test}")
    spark.sql(f"DROP TABLE {pseudo_db_test}.{pseudo}")
    spark.sql(f"""CREATE TABLE {pseudo_db_test}.{pseudo} (
        pseudo STRING, ServiceRequestId STRING, Unique_CareActivityID STRING, PersonBirthDate STRING)"""
        )

    #spark.sql(f"CREATE TABLE IF NOT EXISTS {pseudo_db_test}.{pseudo}")
    ...
    if deid_type == 'pseudo':
        expected_pseudo_view_statements = [f"CREATE VIEW `{target_db}`.`{table1}`(Person_ID, ADSM, ADSM_FirstScore, ServiceRequestId) AS \
                SELECT p0.pseudo AS Person_ID, ADSM, ADSM_FirstScore, sha2(concat(ServiceRequestId, \
                (SELECT salt FROM {pseudo_db_test}.salt \
                WHERE field = 'ServiceRequestId' AND domain IS NULL)), 256) as ServiceRequestId \
                FROM {source_db}.{table1} lhs \
                LEFT JOIN {pseudo_db_test}.pseudo p0 ON p0.domain = 'Domain-1' AND p0.pseudo_type = 'Person_ID' AND lhs.Person_ID = p0.clear ", 
                f"CREATE VIEW `{target_db}`.`{table2}`(Person_ID, ServiceRequestId, Unique_CareActivityID) AS \
                SELECT p0.pseudo AS Person_ID, sha2(concat(ServiceRequestId, (SELECT salt FROM {pseudo_db_test}.salt \
                WHERE field = 'ServiceRequestId' AND domain IS NULL)), 256) as ServiceRequestId, sha2(concat(Unique_CareActivityID, \
                (SELECT salt FROM {pseudo_db_test}.salt WHERE field = 'UniqCareActID' AND domain IS NULL)), 256) as Unique_CareActivityID FROM {source_db}.{table2} lhs \
                LEFT JOIN {pseudo_db_test}.pseudo p0 ON p0.domain = 'Domain-1' AND p0.pseudo_type = 'Person_ID' AND lhs.Person_ID = p0.clear " 
    ...
    if deid_type == 'sensitive':
        expected_sensitive_view_statement = \
            [
                f"CREATE VIEW `{target_db}`.`{table1}`(Person_ID, ADSM, ADSM_FirstScore, ServiceRequestId) AS \
                SELECT p0.pseudo AS Person_ID, ADSM, ADSM_FirstScore, ServiceRequestId FROM {source_db}.{table1} lhs \
                LEFT JOIN {pseudo_db_test}.pseudo p0 ON p0.domain = 'Domain-1' AND p0.pseudo_type = 'Person_ID' AND lhs.Person_ID = p0.clear ",
                f"CREATE VIEW `{target_db}`.`{table2}`(Person_ID, ServiceRequestId, Unique_CareActivityID) AS \
                SELECT p0.pseudo AS Person_ID, ServiceRequestId, Unique_CareActivityID FROM {source_db}.{table2} lhs \
                LEFT JOIN {pseudo_db_test}.pseudo p0 ON p0.domain = 'Domain-1' AND p0.pseudo_type = 'Person_ID' AND lhs.Person_ID = p0.clear "
            ]
